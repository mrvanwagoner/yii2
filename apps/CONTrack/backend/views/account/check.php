...
$this->title = 'Print Check'; //CHANGED MVW 03/22/14 //FIXME Conditional based on type
...
<div class="check-create">
    ...
    <!-- <?= $form->field($model, 'tenant_dbu')->textInput(['maxlength' => 16]) //FIXME Only visible to System Admin and System Staff ?> -->
		...
		<p><?php echo $model->projectDescription; ?></p>

    <p><?php echo Address::getAddress($model->project->x_address_id); ?></p>

    <p><?php echo Address::getCityStatePostalCode($model->project->x_address_id); ?></p>
    ...
    <!-- <?= $form->field($model, 'date_received')->widget(DatePicker::classname(), [
      ...
      'pluginOptions' => [
		...
		<?= $form->field($model, 'net_amount')->textInput(['maxlength' => 19], ['style' => 'text-transform: uppercase'])//CHANGED MVW 03/25/14: Must be an integer ?>

		<?= $form->field($model, 'net_amount')->textInput(['maxlength' => 19], ['style' => 'text-transform: uppercase'])//CHANGED MVW 03/25/14: Must be an integer ?>

    <?php echo '<b>Amount: </b> *** ' . Enum::numToWords($model->net_amount) . ' Dollars and 00/100 **** '; //CHANGED MVW 03/25/14: Must be an integer ?>
